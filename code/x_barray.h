...
namespace x
{
	...
	public:
		...
		barray_long(char const* const origin_char, long long const& origin_char_len);  // char数组初始化barray
		barray_long(unsigned char const* const origin_uchar, long long const& origin_uchar_len);
		barray_long(bool const* const origin_bool, long long const& origin_bool_len);  // bool数组初始化barray，bool数组中True对应1、False对应0，例如bool[3]=True，那么对应barray[3]=0x01。
		barray_long(short const* const origin_short, long long const& origin_short_len);  // short数组初始化barray，short数组中每个short对应barray两个位置，例如short[3]=0xF0F1，那么对应barray[6]=0xF1、barray[7]=0xF0
		barray_long(unsigned short const* const origin_ushort, long long const& origin_ushort_len);
		barray_long(int const* const origin_int, long long const& origin_int_len);  // int数组初始化barray，int数组中每个int对应barray四个位置，例如int[3]=0xF0F1F2F3，那么对应barray[12]=0xF3、barray[13]=0xF2、barray[14]=0xF1、barray[15]=0xF0
		barray_long(unsigned int const* const origin_uint, long long const& origin_uint_len);
		barray_long(long long const* const origin_long, long long const& origin_long_len);    // long long数组初始化barray，long long数组中每个long long对应barray八个位置，例如long long[3]=0xF0F1F2F3F4F5F6F7，那么对应barray[24]=0xF7、barray[25]=0xF6、barray[26]=0xF5、barray[27]=0xF4、barray[28]=0xF3、barray[29]=0xF2、barray[30]=0xF1、barray[31]=0xF0
		...
		// ----- operator=重载 -----
		barray_long& operator=(barray_long const& right_barray);
		barray_long& operator=(char const& right_char);  // 把barray用一个char赋值，barray长度为1，参考用char数组初始化的构造函数
		barray_long& operator=(unsigned char const& right_uchar);
		barray_long& operator=(bool const& right_bool);  // 把barray用一个bool赋值，barray长度为1，参考用bool数组初始化的构造函数
		barray_long& operator=(short const& right_short);  // 把barray用一个short赋值，barray长度为2，参考用short数组初始化的构造函数
		barray_long& operator=(unsigned short const& right_ushort);
		barray_long& operator=(int const& right_int);  // 把barray用一个int赋值，barray长度为4，参考用int数组初始化的构造函数
		barray_long& operator=(unsigned int const& right_uint);
		barray_long& operator=(long long const& right_long);  // 把barray用一个long long赋值，barray长度为8，参考用long long数组初始化的构造函数
		barray_long& operator=(unsigned long long const& right_ulong);

		// ----- operator==重载 -----
		bool operator==(barray_long const& right_barray) const noexcept;
		bool operator==(char const& right_char) const noexcept;
		bool operator==(unsigned char const& right_uchar) const noexcept;
		bool operator==(bool const& right_bool) const noexcept;
		bool operator==(short const& right_short) const noexcept;
		...
		// ----- operator+重载 -----
		barray_long operator+(barray_long const& right_barray) const;  // 若长度超过INT_MAX，只返回到INT_MAX的长度
		barray_long operator+(char const& right_char) const;  // barray往后加1个位置，且值为char，参考用char数组初始化的构造函数
		barray_long operator+(unsigned char const& right_uchar) const;
		barray_long operator+(bool const& right_bool) const;  // barray往后加1个位置，且值为0或1，参考用char数组初始化的构造函数
		barray_long operator+(short const& right_short) const;  // barray往后加2个位置，且值为short，参考用short数组初始化的构造函数
		barray_long operator+(unsigned short const& right_ushort) const;
		barray_long operator+(int const& right_int) const;  // barray往后加4个位置，且值为int，参考用int数组初始化的构造函数
		barray_long operator+(unsigned int const& right_uint) const;
		barray_long operator+(long long const& right_long) const;  // barray往后加8个位置，且值为long long，参考用long long数组初始化的构造函数
		...
		// ----- operator*重载 -----
		barray_long operator*(long long const& multiple) const;  // 若multiple<=0，返回空barray。若长度超过INT_MAX，保留长度在INT_MAX以内的数据
		friend barray_long operator*(int const& multiple, barray_long const& right_barray);

		// ----- operator*=重载 -----
		barray_long& operator*=(long long const& multiple);  // 当multiple<=0时，barray长度为零

		// ----- operator/重载 -----
		barray_long operator/(long long const& divisor) const;  // 若divisor<=0时，返回空barray。若长度超过INT_MAX，保留长度在INT_MAX以内的数据

		// ----- operator/=重载 -----
		barray_long& operator/=(long long const& divisor);  // 当divisor<=0时，barray长度为零

		// ----- operator<<重载 -----
		barray_long operator<<(long long const& offset) const;  // 当offset为正数的时候，则在索引0向后添加offset个值为0x00的字节；当offset为负数的时候，则在最大索引往前添加offset个值为0x00的字节。若barray的长度超过INT_MAX，则优先保存原barray的值，值为0x00的字节则补到长度为INT_MAX为止

		// ----- operator<<=重载 -----
		barray_long& operator<<=(long long const& offset);

		// ----- operator>>重载 -----
		barray_long operator>>(long long const& offset) const;  // 当offset为正数的时候，则在索引0向前删除offset个字节；当offset为负数的时候，则在最大索引向后删除offset个字节。offset的绝对值大于原barray的长度，则返回长度为0的barray
